---
type: skill
id: aqa-chem-4-1-3-1-comparison-with-group-1-elements
title: 4.1.3.1 Comparison with Group 1 elements
quadrant: Paper 1
ring: Higher combined
categoryId: p1-atomic-structure
levelId: higher-combined
order: 2014
prereqs:
  - aqa-chem-4-1-3
---

**Spec reference:** 4.1.3.1 (under 4.1.3).


## What you need to know

- Reduction is gaining of electrons.
- Metals that are below hydrogen in the reactivity series do not react with dilute
- acids.

> KO match: ** H₂**


## Support videos
- FreeScienceLessons (topic playlist): (use links below)
- FreeScienceLessons (Atomic structure playlist): https://www.youtube.com/playlist?list=PL9IouNCPbCxULWXCO9jt0PsuAbxYpw2_1

## Self-assessment (checklist)

- I can predict properties from given trends down the
- I can describe how the transition metals compare with

## Key steps (method)

- Identify what the question is testing and use precise chemistry vocabulary.
- If it’s a calculation: write the equation/formula first, then substitute carefully.

## Common mistakes

- Missing units/state symbols when asked.
- Giving a statement without linking cause → effect.

## Exam-style questions (quick)

1) Define the key term(s) for this point in the spec.
2) Explain a typical property/result using particle/bonding ideas.
3) Do a short calculation or interpretation step relevant to this point.

## Practice questions
- PMT: Atomic structure (questions + notes): https://www.physicsandmathstutor.com/chemistry-revision/gcse-aqa/atomic-structure-and-periodic-table

## Extra knowledge organisers


## Revision links

- AQA spec section: https://www.aqa.org.uk/subjects/chemistry/gcse/chemistry-8462/specification/subject-content/atomic-structure-and-the-periodic-table
